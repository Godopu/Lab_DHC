...
public class ResourceManager extends Thread {
	...
	private void eventProcess(ArrayList<Object> receivedArrayList) {
		...
		case "requestMgmtCmdControl":
			...
			requestMgmtCmdControl((ExecInstance) receivedArrayList.get(1));
			break;
			
		case "requestMgmtCmdFind":
			if (debugPrint) {
				System.out.println("[ResourceManager] Receive MgmtCmd Find request");
			}
			requestMgmtCmdFind((ExecInstance) receivedArrayList.get(1));
			break;
		...
		case "requestCSEBaseRetrieve":
			if (debugPrint) {
				System.out.println("[ResourceManager] Receive Retrieve CSEBase information...");
			}
			requestCSEBaseRetrieve();
	...
	private void mgmtCmdRegistration(MgmtCmd receivedThingControl) {
		...
		if (tempMgmtCmd.resourceName.equals("deviceManagement") ||
			tempMgmtCmd.resourceName.equals("firmwareUpgrade") ||
			tempMgmtCmd.resourceName.equals("appInstall")) {
			
			registMgmtCmd.add("registMgmtCmd");
			registMgmtCmd.add(tempMgmtCmd.resourceName);
	 ...
	 */
	private void requestMgmtCmdControl(ExecInstance receivedMgmtCmdControl) {
		...
		if (numberOfMgmtCmd > 0) {
			for (int i = 0; i < numberOfMgmtCmd; i++) {
				...
				if (receivedMgmtCmdControl.mgmtCmdName.equals("deviceManagement") ||
					receivedMgmtCmdControl.mgmtCmdName.equals("firmwareUpgrade")) {
					
					ArrayList<Object> deviceSendArrayList = new ArrayList<Object>();
					deviceSendArrayList.add("requestMgmtCmdControl");
					deviceSendArrayList.add(receivedMgmtCmdControl.mgmtCmdName);
				...
				else if (receivedMgmtCmdControl.mgmtCmdName.equals("appInstall")) {
					ArrayList<Object> applicationSendArrayList = new ArrayList<Object>();
					applicationSendArrayList.add("requestMgmtCmdControl");
					applicationSendArrayList.add(receivedMgmtCmdControl.mgmtCmdName);
				...
				else if (receivedMgmtCmdControl.mgmtCmdName.equals(tempMgmtCmd.resourceName)) {
					...
					if (debugPrint) {
						...
						System.out.println(tempMgmtCmd.resourceName);
					}
				}
			}
		}
	...
	private void requestMgmtCmdFind(ExecInstance receivedMgmtCmdControl) {
		
		int numberOfMgmtCmd = resourceDB.getNumberOfMgmtCmd();
		
		boolean findMgmtCmd = false;
		...
		if (numberOfMgmtCmd > 0) {
			for (int i = 0; i < numberOfMgmtCmd; i++) {
				...
				if (tempMgmtCmd.resourceID.equals(receivedMgmtCmdControl.parentID)) {
					ArrayList<Object> httpServerSendArrayList = new ArrayList<Object>();
					httpServerSendArrayList.add("Found");
					httpServerSendArrayList.add(tempMgmtCmd.resourceName);
					findMgmtCmd = true;
		...
		if (!findMgmtCmd) {
			ArrayList<Object> httpServerSendArrayList = new ArrayList<Object>();
			httpServerSendArrayList.add("not found");
	...
	private void requestCSEBaseRetrieve() {
		ArrayList<Object> httpServerSendArrayList = new ArrayList<Object>();
		
		httpServerSendArrayList.add("requestCSEBaseRetrieve");
		httpServerSendArrayList.add(resourceDB.getCSEProfile());
		
		try {
			if (debugPrint) {
				System.out.println("[ResourceManager] Send to HttpServer - CSEBase Info retrieve response");
				System.out.println();
			}
