...
public class ThingInteractionProcess extends Thread {
	...
	public void requestThingRegistration(Container receivedThingProfile) throws Exception {
		...
		for (int i = 0; i < numberOfThings; i++) {
			...
			if (tempThingProfile.resourceName.equals(receivedThingProfile.resourceName)) {
				registThing = true;
				break;
			}
		...
		if (!registThing) {
			...
			tempProfile.resourceName = receivedThingProfile.resourceName;
			...
			responseString = "registSuccess;" + receivedThingProfile.resourceName +"," + receivedThingProfile.resourceID;
		}
		else {
			System.out.println("[ThingManager] Thing Profile is already registerd");
			responseString = "registFailed;" + receivedThingProfile.resourceName +"," + receivedThingProfile.resourceID + ";AlreadyRegisterd";
		}
	...
	public void requestThingControlRegistration(MgmtCmd receivedThingControl) throws Exception {
		...
		for (int i = 0; i < numberOfThingControls; i++) {
			...
			if (tempThingControl.resourceName.equals(receivedThingControl.resourceName)) {
		...
		if (!registThingControls) {
			...
			responseString = "registSuccess;" + tempThingControl.resourceName;
		}
		else {
			if (debugPrint) System.out.println("[ThingManager] Thing Control is already registerd");
			responseString = "registFailed;" + receivedThingControl.resourceName + ";AlreadyRegisterd";
		}
