...
public class InteractionManager extends Thread {
	...
	private MqttClientKeti mqttMobiusClient;
	private String requestFrom = null;
	private String inCSEAddress = null;
	
	// Next release...
	@SuppressWarnings("unused")
	private MqttClientKeti mqttOneM2MClient;
	
	private static final boolean interopType = DeviceManager.getInteropType();
	private static final boolean primitiveType = DeviceManager.getPrimitiveType();
	private static final boolean debugPrint = DeviceManager.getDebugPrint();
	
	// Next release...
	@SuppressWarnings("unused")
	private static final boolean protocolBinding = DeviceManager.getPrimitiveType();
	
	private InteractionHTTPRequest interactionRequest;
	...
	public InteractionManager(
		...
		this.mqttMobiusClient = new MqttClientKeti(mqttServerUrl, interactionManagerQueue);
		this.mqttOneM2MClient = new MqttClientKeti(mqttServerUrl, interactionManagerQueue);
		this.interactionRequest = new InteractionHTTPRequest(inCSEAddress, interopType, primitiveType, debugPrint);
		this.interactionResponseParser = new InteractionResponseParser(primitiveType);
	}
	...
	public void setCSERegistration(CSEBase receivedProfile) throws Exception {
		...
		String[] returnedProfile = interactionRequest.CSERegistrationMessage(receivedProfile);
		
		if (returnedProfile[0].length() > 3) {
			CSEBase updateProfile = interactionResponseParser.CSERegistration(receivedProfile, returnedProfile);
			...
			mqttMobiusClient.subscribe(updateProfile.CSEID);
			...
			if (debugPrint) {
				...
				System.out.println("[InteractionManager] MQTT Client Subscribe : " + receivedProfile.CSEID);
			}
			mqttMobiusClient.subscribe(receivedProfile.CSEID);
	...
	public void setMgmtCmdControl(String receivedControlData) throws Exception {
		...
		String mgmtCmdName = st.nextToken();
		String execInstanceBody = st.nextToken();
		
		while (st.hasMoreTokens()) {
			execInstanceBody = execInstanceBody + "," + st.nextToken();
		}
		
		execInstanceBody = execInstanceBody.substring(55);
		
		ExecInstance mgmtCmdUpdateRequest = interactionResponseParser.execInstanceParse(execInstanceBody);
		mgmtCmdUpdateRequest.mgmtCmdName = mgmtCmdName;
	 ...
	 */
	public void setThingControlStatus(ExecInstance receivedThingControl,
